# -*- coding: utf-8 -*-
"""This module defines the helpers function for the animation computation"""
...
def load_img(path, shape):
    """Load image either from an url, or a local file, and return it as a tensor of a specified shape

    Args:
        path (str): the url or path of the image file
        shape (tuple of int, int): the shape of the tensor to return

    Returns:
        tensor: the image as a tensor of the specified shape"""
...
def anim_frame_warp_3d(prev_img_cv2, depth, anim_args, keys, frame_idx, device):
    """Warp the animation frame in 3D space"""
...
def transform_image_3d(
    ...
    Adapted and optimized from Disco Diffusion https://github.com/alembics/disco-diffusion"""
...
def sample_from_cv2(sample: np.ndarray):
    """Sample from cv2 to torch tensor"""
...
def sample_to_cv2(sample, type=np.uint8):
    """Sample from torch tensor to cv2"""
...
def maintain_colors(prev_img, color_match_sample, mode):
    """Tries to match color of previous frame

    Args:
      prev_img (Image): the previous generated frame
      color_match_sample (Image): the first generated frame
      mode (str): can be "Match Frame 0 RGB", "Match Frame 0 HSV", or "Match Frame 0 LAB"
                  the algorithm will try to minimize the distance in the selected color space and mode

    Returns:
      Image: image with color coherence
    """
...
def next_seed(args):
    """Changes seed based on behavior defined in args"""
...
def get_uc_and_c(prompts, model, args, frame=0):
    """Returns uc and c for weighted prompts"""
